...
"""Create a movement timeline from an RGB video using frame differences."""
...
def latest_video() -> Path:
    files = list(ROOT.glob("session_*/raw_rgb/*.mp4"))
    if not files:
...
def main():
    ...
    while True:
        ...
        if previous is not None:
            ...
            signed -= np.median(signed)  # reject global brightness changes
    ...
    output = args.output or args.video.parent.parent / "movement_rgb.png"
    fig, ax = plt.subplots(figsize=(14, 5))
    ax.plot(times / 60, smooth, color="#d94801", linewidth=1.5)
    ax.set(title="Movement Timeline — RGB Frame Difference", xlabel="Elapsed time (min:sec)", ylabel="Mean absolute frame difference (% intensity)")
